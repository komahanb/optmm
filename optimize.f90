      subroutine optimize(ndvar,D,ndvart,fobj,dfdD,low,up,gtol,maximize,outputscreen,fct)
        ...
        double precision :: D(ndvart),fobj,dfdD(ndvar),gtol,low(ndvar),up(ndvar)
...
!     This is the call to the L-BFGS-B code.
        ...
        call setulb(ndvar,mlim,D,low,up,nbd,fobj,dfdD,factr,pgtol,wa,iwa,task,iprint,csave,lsave,isave,dsave)
 

        if (task(1:2) .eq. 'FG') then
!        the minimization routine has returned to request the
!        function f and gradient dfdD values at the current D.


         
           call Calcstuff(D,ndvart,fobj,dfdD,fct)
